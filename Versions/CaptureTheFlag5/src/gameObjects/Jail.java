...
public class Jail implements GameObject
{
    ...
    public String pack()
    {
        ...
        for( int i = 0; i < blue.size(); ++i ) {
            ...
            String criminal = "B" + blue.get( i ).getKey().getID() + "=" + blue.get( i ).getValue() + ",";
            data += criminal;
        }
        ...
        for( int i = 0; i < red.size(); ++i ) {
            // criminal = index of player + his time in jail
            // TODO; time may be too long as it is the highest precision double... maybe convert to int for network transfer?
            String criminal = "R" + red.get( i ).getKey().getID() + "=" + red.get( i ).getValue() + ",";
            data += criminal;
        }
    ...
    {
        // split data into different criminals
        ...
        for( int i = 0; i < parts.length; ++i ) {
            ...
            // index of equal sign / separator
            int equals = criminal.indexOf( "=" );

            // jail red / blue
            String jail = criminal.substring( 0 , 1 );

            // string containing player index
            String strIndex = criminal.substring( 1, equals );

            // string containing player jail time
            String strTime = criminal.substring( equals + 1 );
            ...
            // is blue jail member?
            if( jail == "B" ) {
                // loop blue jail and find player
                for( int x = 0; x < blue.size(); ++x ) {
                    // pair of player and timer
                    Pair< Player, Double > pair = blue.get( x );

                    // is this the right pair?
                    if( pair.getKey().getID() == index ) {
                        // create a new updated pair
                        Pair< Player, Double > updated = new Pair< Player, Double >( pair.getKey(), time );
            ...
            // is red jail member?
                ...
                // loop red jail and find player
                for( int x = 0; x < red.size(); ++x ) {
                    // pair of player and timer
                    Pair< Player, Double > pair = red.get( x );
    ...
    public void draw( Graphics g, int scaleDownFactor, int offsetX, int offsetY )
    {
        ...
        g.fillRect( x / scaleDownFactor + offsetX, y / scaleDownFactor + offsetY, WIDTH / scaleDownFactor, HEIGHT / scaleDownFactor );
    }
